...
### Purpose
Stores user metadata (roles, flat numbers, additional information) for authorization. This application uses **Supabase Auth** for authentication, with the `users` table storing additional user metadata linked via `user_id`.
...
| `password_hash` | TEXT | NULL | - | - | - | **Deprecated**: Password hashing is handled by Supabase Auth. This field may be present for backward compatibility but is not used. |
...
### Login Process (Hybrid Approach)

1. **User Input**: Email/mobile + password
2. **Supabase Auth Login**: Authenticate using `supabase.auth.signInWithPassword()`
   - If mobile number is provided, first lookup email from `users` table by `mobile_number`
   - Supabase Auth handles password verification (bcrypt hashing)
   - Returns JWT token and session
3. **Fetch User Metadata**: Query `users` table using `user_id` from Supabase Auth user
4. **Store Session**: Supabase handles secure session storage (encrypted tokens)
5. **Set Permissions**: Load role-based permissions from `users` table

### Password Hashing

- **Handled by Supabase Auth**: Automatic bcrypt password hashing
- **No manual hashing required**: Supabase Auth manages all password security
- **Flutter Implementation**: Uses `supabase_flutter` package
