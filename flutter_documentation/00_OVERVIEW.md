...
### Hybrid Authentication (Supabase Auth + Users Table)

This application uses a **hybrid authentication approach**:
- **Supabase Auth** for authentication (password hashing, session management, JWT tokens)
- **`users` table** for user metadata (roles, flat numbers, additional user information)

### Authentication Flow

1. **Login**: User provides email/mobile + password
2. **Supabase Auth**: Handles password verification and JWT token generation
3. **User Metadata**: After authentication, user details are fetched from `users` table
4. **Session Management**: Supabase handles session storage (secure, encrypted tokens)

### Password Hashing

- **Handled by Supabase Auth**: Automatic password hashing using bcrypt
- **No manual hashing required**: Supabase Auth handles all password security
- **Flutter Implementation**: Uses `supabase_flutter` package

```dart
// Example login (handled by Supabase Auth)
final response = await supabase.auth.signInWithPassword(
  email: email,
  password: password,
);

// Then fetch user metadata from users table
...
**Note**: Roles are stored in the `users` table, not in Supabase Auth user metadata.
